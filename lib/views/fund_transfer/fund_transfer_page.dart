...
class _FundTransferPageState extends State<FundTransferPage>
      ...
      child: Scaffold(
        ...
        body: MyScrollView(
          keyboardConfig: Utils.getKeyboardActionsConfig(
              context, <FocusNode>[_nodeText1,_nodeText3, _nodeText2]),
  ...
  List<Widget> _buildBody() {
    return [
      ...
      Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              InkWell(
                onTap: () => {availableBalace(_debitAccount.id)},
                child: Padding(
      ...
      Padding(
        padding: const EdgeInsets.all(4.0),
        child: Text(
          AppLocalizations.of(context).fundTransferAmountLimit,
          style:
              TextStyle(fontSize: Dimens.font_sp10, color: Colours.text_gray),
        ),
      ),
      Gaps.vGap8,
      MyTextField(
  ...
  void _next() async {
    ...
    for (var category in _categoiesList) {
      if (category.type ==
          TransactionType.FundTransfer.toString().split('.').last) {
        id = category.id;
        break;
      }
    }
    var response = await presenter.transactionFeeCalculate(id, amount,vendor: _creditAccount.vendor);
    if (response != null) {
      txnFees = response;
      ...
      showConfirmation(amount, double.parse(feeAmount), double.parse(vatAmount),
          purpose, cvv,isOtpRequired);
    }
  }

  Future<bool> showConfirmation(double amount, double feeAmount,
      double vatAmount, String purpose, String cvv,bool isOtpRequired) async {
    final result =
        await Navigator.push(context, MaterialPageRoute(builder: (context) {
      return FundTransferConfirmationPage(
          amount,
          feeAmount,
          vatAmount,
  ...
  void _selectReceiver() {
    showModalBottomSheet(
        ...
        builder: (_) => AllAccountSelectorPage(
              Constant.transactionAccountSelector,
              'fund_transfer',
              selectedAccountId: _debitAccount?.id,
            ));
  }

  @override
  FundTransferPresenter createPresenter() {
    return new FundTransferPresenter();
  }
